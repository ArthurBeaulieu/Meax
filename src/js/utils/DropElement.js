class DropElement {
  ...
  destroy() {
    if (this._evtIds.length > 0 && this._evtIds) {
      for (let i = 0; i < this._evtIds.length; ++i) {
        window.CustomEvents.removeEvent(this._evtIds);
      }
    }
  ...
  _events() {
    this._evtIds.push(window.CustomEvents.addEvent('dragenter', this._target, this._dragEnter, this));
    this._evtIds.push(window.CustomEvents.addEvent('dragover', this._target, this._dragOver, this));
    this._evtIds.push(window.CustomEvents.addEvent('dragleave', this._target, this._dragLeave, this));
    this._evtIds.push(window.CustomEvents.addEvent('drop', this._target, this._drop, this));
    this._evtIds.push(window.CustomEvents.addEvent('touchmove', document.body, this._dragTouchOver, this));
    this._evtIds.push(window.CustomEvents.addEvent('touchend', document.body, this._dragTouchEnd, this));
  }
  ...
  _drop(event) {
    ...
    for (let i = 0; i < files.length; ++i) {
      ...
      reader.onload = (file => {
        return raw => {
          this._onDropCB(file, JSON.parse(raw.target.result))
        };
      })(files[i]);
